...
static const USBDescriptor *get_descriptor(USBDriver *usbp,
  ...
  usb_debug_putX('t');
  usb_debug_phexX(dtype);
  usb_debug_putX('i');
  usb_debug_phexX(dindex);
#endif /* DEBUG_USB */
  switch (dtype) {
...
int main(void) {
  ...
  /* Turn off the LED on TBD. */
  palSetPad(GPIO_LED, PIN_LED);
  chThdSleepMilliseconds(500);

  /* Pins for LPUART0 */
  palSetPadMode(IOPORT4, 6, PAL_MODE_ALTERNATIVE_3);
  palSetPadMode(IOPORT4, 7, PAL_MODE_ALTERNATIVE_3);
  ...
  while (!chThdShouldTerminateX()) {
    ...
    chThdSleepMilliseconds(1000);
    palTogglePad(GPIO_LED, PIN_LED); /* LED */
  }
